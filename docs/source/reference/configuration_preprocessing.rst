...
.. list-table::
   ...
   * - ``ic_label``
     - Original MNE-ICALabel MEGNet, optional retrained MEGNet, MNE ECG/EOG,
       and rule-based component labeling.
   * - ``epochs``
...
The ``ic_label`` block combines four selector families. The original and
retrained MEGNet classifiers have independent switches and may be enabled at
the same time:
...
``marked_components.txt`` before ICA application. When multiple detectors
assign the same component to ECG or EOG, ``ecg_eog_scores.json`` stores the
maximum score in its summary fields and retains each MEGNet method's full
four-class probabilities under ``methods``. Its ``marked_components`` entry
also records the latest automatic union, the indices actually used, and whether
an existing manual edit was preserved during a safe Nextflow refresh.
